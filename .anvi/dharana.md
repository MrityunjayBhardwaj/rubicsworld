...
**Fatality status:** FATAL (8 patterns clustered — P8, P21, P23, P24, P31, P33, P34, P35). Pattern shape is "custom render path makes an assumption the library side silently disagrees with" — extended this session to "any consumer of the scene graph other than the renderer (bake/serialise/inspect) hits the same class of bugs." **Strongest smell at this boundary**: uniforms / authoring values probe as correct but the consumer (GPU output, exported glb, walking player) behaves as if they're at defaults — the "my state says X, observer sees Y" gap. Default next step at this boundary = force the channel explicitly: bind depth/color textures directly to effect materials; copy authoring scalars from hidden mesh to visible mesh; patch glb metadata server-side; route bakes through a non-rendering page so headless WebGL doesn't stall. **Session-2026-04-25 additions**: P31 (hidden authoring channel to visible mesh — sphere-terrain PBR), P33 (BufferAttribute.name dead-write at glTF export — patch JSON post-export), P34 (texture undecoded at export — strip or await), P35 (24-tile sphere render stalls headless WebGL — bake via `/bake/` non-rendering page).
