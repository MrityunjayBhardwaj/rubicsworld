...
**REF:** UNGROUNDED — historical diagnosis from `src/world/PostFx.tsx` when N8AO + SSAO ran in the chain; both stripped 2026-Q1 (file header comment confirms). Underlying mechanism unchanged: `src/diorama/TileGrid.tsx:patchMaterialForSphere` displaces vertices; N8AO's neighbour-delta normal reconstruction in `node_modules/n8ao/dist/N8AO.js` assumes smooth depth. Rule applies if AO is reintroduced — pick SSAO over N8AO for this scene.
...
## P39: Pre-flip in one layer masks asymmetric sign conventions in downstream layers
**Root cause:** `buildCellDefs.localV = 1 - (row % 2)` flipped the row→localV mapping at the source. Two downstream consumers used OPPOSITE vOff sign formulas — `cubeCellRender` (`(localV - 0.5) * CELL`) and `storeTileCubeRender` (`(0.5 - localV) * CELL`). Both happened to render content at the same physical position only because the pre-flip cancelled out exactly one of the two asymmetries. The system was secretly inconsistent but visually consistent.
**Detection signal:** Change `buildCellDefs.localV` to `row % 2` to "fix" the convention → cell-based path moves correctly but store-tile path now diverges. Or: notice that two consumers reading the same logical input use formulas with opposite signs.
**The trap:** Treating the surface symptom as the bug (e.g. flipping just one consumer). Root fix: unify ALL v-position consumers on a single sign convention BEFORE removing the pre-flip. Sites that must agree: `cubeCellRender`, `storeTileCubeRender`, `buildOverlayLines` (cube + sphere branches), `CubeLabels`, `SphereLabels`, `audio/sphereProject.cubeNetToSphere`, `tileCentroid` + `centroidToFaceUV`.
**REF:** UNGROUNDED — canonical instance `src/diorama/TileGrid.tsx:buildCellDefs` + `cubeCellRender` + `storeTileCubeRender`.

## P40: tileCentroid and isSolved encoded different v-axis frames — latent inconsistency masked by internal self-consistency
**Root cause:** `tileCentroid` (rotation.ts) used `t = 1 - (2v+1)/N` (v=0 → +face.up). `isSolved` (tile.ts) inlined `tH = -1 + (2v+1)/N` (homeV=0 → -face.up). Both functions worked correctly within themselves: isSolved compares its own homeC to its own curC using the same sign, so internal equality holds. But across the two functions, a tile at the same store state has centroids in mirror frames. Slice rotation hit-testing (uses tileCentroid) and solved-state checking (uses isSolved frame) operate in different coordinate systems.
**Detection signal:** Searching for "convention violations": grep for `(2 * v + 1) / N` produces two formulas with opposite signs. Or: any extension that crosses from rotation.ts's frame into tile.ts's frame produces wrong results.
**The trap:** Fixing only one site or assuming "they were both working before so they must agree." Root fix: pick one frame, apply everywhere. (Alternative: keep them independent and document loud that cross-function vector comparison is invalid — but that's fragile.)
**REF:** UNGROUNDED — canonical instance `src/world/rotation.ts:tileCentroid` and `src/world/tile.ts:isSolved`.

## P41: Blender → glTF Y/Z-up conversion silently swaps content along the flat-net's depth axis
**Root cause:** Blender glTF exporter (with default "+Y Up" ON) applies an equivalent of `rotation_x = -90°` to the scene: `Blender (X, Y, Z) → three (X, Z, -Y)`. For a flat-plane diorama authored on Blender's XY plane, Blender +Y (visual north in top view) becomes three -Z. If the flat-net's "north" corresponds to one face block (e.g. C at rows 4-5 in three's +Z) and "south" to another (D at rows 0-1 in three's -Z), then content authored as "C" lands where "D" expects to be, and vice versa. A and F (along X axis) are unaffected because X is identity under the conversion.
**Detection signal:** Diorama loaded via `?glb=1` shows specific face contents swapped between visually-symmetric face pairs along the converted axis (Y in Blender → Z in three). Other faces look correct.
**The trap:** Trying to fix it with `gltf.scene.scale.z = -1` (mirror flips chirality, requires winding reversal everywhere) or `rotation.y = π` (also flips X axis, swapping different faces). Cleanest fix: re-author the Blender face-block guide with labels arranged for the post-conversion orientation, OR encode the rotation at a single boundary (the FACES table — see P42 for the cascade).
**REF:** UNGROUNDED — Blender source: glTF exporter axis-conversion logic. Project instance: `src/diorama/loadGlbDiorama.ts` + flat-net `FACE_TO_BLOCK_TL` in `src/diorama/DioramaGrid.tsx`.

## P42: Face-basis edits in FACES require a paired update to `centroidToFaceUV`'s axis dispatch
**Root cause:** `centroidToFaceUV` uses a hardcoded axis-dominance dispatch: `if ax >= ... face = c.x > 0 ? 0 : 1; else if ay >= ... face = c.y > 0 ? 2 : 3; else face = c.z > 0 ? 4 : 5`. This encodes "face 2 lives at +Y, face 3 at -Y," etc. If you rotate FACES[2] and FACES[3] so face 2 is now physically at -Y (and vice versa), the dispatch silently picks the wrong face for every centroid that lands in those slots. NEIGHBOR_IDX (built at module load via centroidToFaceUV) propagates the error into the neighbor table. Slice rotation appears to work for axes that don't cross C/D, then explodes the moment a rotation lands a tile on the swapped pair.
**Detection signal:** After a FACES basis edit, slice rotations involving Y-axis or any rotation that moves tiles onto C/D produce garbage tile placement. Tile.face values disagree with the visual position.
**The trap:** Looking at `cubeCellRender` and `storeTileCubeRender` and seeing they use `face.normal/right/up` automatically. Yes, the FORWARD direction (face index → 3D position) auto-cascades. The INVERSE direction (3D position → face index) does NOT — it's a hand-coded switch. Root fix: when changing which face index sits at which axis sign, also flip the corresponding `c.{x|y|z} > 0 ? A : B` ternary.
**REF:** UNGROUNDED — canonical instance `src/world/rotation.ts:centroidToFaceUV` (lines 28-33). Caller: `src/world/rotation.ts:NEIGHBOR_IDX` (auto-rebuilds at load) + slice-rotation pipeline.
